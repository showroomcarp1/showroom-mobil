...
// Skeleton loading (Disesuaikan untuk presisi 2 kolom di mobile)
export function CarCardSkeleton({
  ...
  if (variant === "compact") {
    return (
      <div
        aria-hidden="true"
        className="flex flex-col items-center text-center p-1.5 sm:p-2 rounded-md animate-pulse w-full"
      >
        <div className="h-20 w-28 sm:h-36 sm:w-56 bg-neutral-200 rounded-md sm:rounded-none mb-1.5" />
        <div className="h-3 w-16 sm:w-28 bg-neutral-200 rounded-xs" />
      </div>
    );
  }
  ...
  return (
    <div
      aria-hidden="true"
      className="flex flex-col w-full bg-white p-2 sm:p-4 rounded-md animate-pulse border border-neutral-100 sm:border-none"
    >
      {/* Aspect ratio disesuaikan persis dengan gambar utama */}
      <div className="aspect-4/3 w-full bg-neutral-200 rounded-t-md sm:rounded-none" />

      <div className="mt-2 sm:mt-3 flex flex-col flex-1 justify-between">
        <div>
          {/* Skeleton Judul Mobil (2 baris di mobile) */}
          <div className="h-3.5 sm:h-5 bg-neutral-200 rounded-xs w-full mb-1" />
          <div className="h-3.5 sm:h-5 bg-neutral-200 rounded-xs w-2/3 mb-2" />

          {/* Skeleton Metadata (Tahun | Mileage | Transmisi) */}
          <div className="flex items-center gap-1 sm:gap-2">
            <div className="h-2.5 sm:h-3 bg-neutral-200 rounded-xs w-8 sm:w-12" />
            <div className="h-2 w-[1px] bg-neutral-200" />
            <div className="h-2.5 sm:h-3 bg-neutral-200 rounded-xs w-10 sm:w-16" />
            <div className="h-2 w-[1px] bg-neutral-200" />
            <div className="h-2.5 sm:h-3 bg-neutral-200 rounded-xs w-8 sm:w-12" />
          </div>
        </div>

        {/* Skeleton Footer (Harga) */}
        <div className="mt-2.5 sm:mt-3.5 pt-2 border-t border-neutral-200 space-y-1">
          <div className="h-2 sm:h-3 bg-neutral-200 rounded-xs w-16 sm:w-24" />
          <div className="h-4 sm:h-6 bg-neutral-200 rounded-xs w-20 sm:w-32" />
        </div>
      </div>
    </div>
  );
...
// Card utama dengan HTML Semantik
    ...
    >
      ...
      >
        ...
        <section className="mt-2 sm:mt-3 flex flex-col flex-1 justify-between p-2 sm:p-0">
          <header>
            <h3 className="text-xs sm:text-[18px] font-bold text-neutral-950 leading-snug tracking-tight transition-colors line-clamp-2 min-h-[2rem] sm:min-h-[2.6rem] uppercase">
              {displayTitle}
            </h3>

            {metadataItems.length > 0 && (
              <div className="mt-1 sm:mt-2 flex flex-wrap items-center text-[9px] sm:text-xs text-neutral-500 font-medium uppercase tracking-wider">
          ...
          <footer className="mt-2 sm:mt-3.5 pt-1.5 sm:pt-2.5 border-t border-neutral-200">
            <span className="block text-[9px] sm:text-[11px] font-semibold text-neutral-500 uppercase tracking-wider leading-none">
              Prices Starting From
            </span>
            <div className="mt-1 flex flex-wrap items-baseline gap-1 sm:gap-2">
              <p className="text-sm sm:text-xl font-bold text-red-600 tracking-tight leading-tight">
                {formattedFinalPrice}
              </p>
              {hasDiscount && (
