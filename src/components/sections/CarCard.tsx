...
if (variant === "compact") {
return ( <div className="flex flex-col items-center text-center p-2 rounded-md animate-pulse"> <div className="h-24 w-32 sm:h-36 sm:w-56 bg-neutral-200 rounded-md sm:rounded-none mb-2" /> <div className="h-3 w-20 sm:w-28 bg-neutral-200 rounded-sm" /> </div>
);
}

return ( <div className="flex flex-col w-full bg-white p-3 sm:p-4 rounded-md animate-pulse"> <div className="aspect-4/3 w-full bg-neutral-200 rounded-md sm:rounded-none" /> <div className="mt-3 sm:mt-3 flex flex-col flex-1 justify-between"> <div> <div className="h-4.5 sm:h-5 bg-neutral-200 rounded-sm w-5/6 mb-1.5" /> <div className="h-4.5 sm:h-5 bg-neutral-200 rounded-sm w-2/3" />
...
// Helper untuk format teks transmisi
...
return ( <span> <span className="inline sm:hidden">{mobileText}</span> <span className="hidden sm:inline">{transmission}</span> </span>
);
}
...
{car.condition && ( <div className="absolute top-0 left-0 z-10 pointer-events-none scale-75 origin-top-left"> <CarBadge condition={car.condition} /> </div>
)}
...
> <Link
   ...
   >
{/* Gambar utama */} <figure className="relative aspect-4/3 w-full overflow-hidden flex items-center justify-center rounded-t-md sm:rounded-none">
{car.condition && ( <div className="absolute top-0 left-0 z-10 pointer-events-none"> <CarBadge condition={car.condition} /> </div>
)}
...
```
      ...
      <Image
        ...
        decoding="async"
        quality={70}
        sizes="(max-width: 640px) 50vw, (max-width: 1024px) 33vw, (max-width: 1280px) 25vw, 330px"
        className="object-cover"
        placeholder="empty"
        unoptimized={isDataImage}
      />
